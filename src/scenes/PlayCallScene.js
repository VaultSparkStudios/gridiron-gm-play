...
const CALLS = [
  ...
  { id:'run_draw',    label:'Draw Play',    cat:'run',  tip:'RB delays — freezes defense' },
...
export class PlayCallScene extends Phaser.Scene {
  ...
  create() {
    ...
    const btnW=154, btnH=42, startY=py-panelH/2+65;
    const runs   = CALLS.filter(c=>c.cat==='run');
    const passes = CALLS.filter(c=>c.cat==='pass');

    this.add.text(px-92, startY-14, '🦶 RUN', { fontSize:'9px', fontFamily:'monospace', fontStyle:'bold', color:'#f59e0b' }).setOrigin(0.5,0).setDepth(32);
    this.add.text(px+92, startY-14, '🏈 PASS',{ fontSize:'9px', fontFamily:'monospace', fontStyle:'bold', color:'#3b82f6' }).setOrigin(0.5,0).setDepth(32);

    runs.forEach((c,i)   => this._makeBtn(c, px-92, startY+i*(btnH+5), btnW, btnH, '#f59e0b'));
    passes.forEach((c,i) => this._makeBtn(c, px+92, startY+i*(btnH+5), btnW, btnH, '#3b82f6'));
  }
